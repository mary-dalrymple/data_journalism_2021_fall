...
**Answer** Screenshot of a set of files that appears to have a shapefile, a projection file, an xml text file, and some other stuff of a kind described as "document."
...
**Answer** The shapefile is described as a "simple feature collection" with 7,481 features and 32 fields -- but also 7,571 rows and 33 columns. (Not sure yet how those attribute/observation labels differ.) The shapefile appears to have GPS points that define its limits. 

The columns contain hospital data that includes address and phone, status (open/closed), population, lat/long, NAICS code and description for hospital type, information sourcing, owner type, beds, trauma level, and more!
...
**Answer** This created a dataframe where state = MD, indicating 72 hospitals in Maryland. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below
**Answer** Maryland has 50 "general "regular" acute care hospitals, 11 psychiatric, 4 military, 2 chronic disease, 2 long-term care, 2 rehab and 1 "special." A couple of these, including the "special" hospital, appear to be closed. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** A dataframe of the 50 hospitals categorized as "general acute care" hospitals. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** They're roughly concentrated in Baltimore and then scattered, with more density in what appears to be Montgomery and Prince George's counties. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see the suggestion to cache. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I get a dataframe of 3,233 rows and 18 columns, incluing what appear to be FIPS codes, GEOIDs to match census files, geometry data, and a bunch of other stuff.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I get a new dataframe of 24 rows and 18 columns, limited to Maryland counties.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see a greyscale verion of Maryland with county borders. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see the greyscale Maryland with black dots representing the hospital locations. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see a white Maryland, counties marked, and hospital points colored to match bed count (where most hospitals are under 250 beds, and then a smaller proportion between 250-500 beds).
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This produces the same dataframe of 50 general acute care hospitals in Maryland.
...
# why convert to a tibble?

### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see a dataframe of 22 rows, one for each county with at least one hospital, that shows the sum of hospital beds in the county.  
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This returned a dataframe of 24 jurisdictions in Maryland and their estimated populations.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see the same dataframe we pulled just above, now with a new Geometry column appended. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Two dataframes showing the aforementioned issues in the column with the county text label.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This produces a transformed dataframe with three columns -- COUNTY to match the md_beds_by_county dataframe; population; geometry.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** A single dataframe with county, population, total hospital beds, and geometry. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Same dataframe as described above with 0 in the total_beds column for Queen Anne's and Carolina counties. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Same dataframe with a new column, beds_per_100k, that has Baltimore City at the top and our 0-bed counties at the bottom. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Inky blue Maryland, where lighter colors indicate a higher number of beds per 100,000 people, with Balitmore City taking the lead. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** A more differentiated Maryland. It's still clear that Balitmore City ranks highest for beds per 100,000 people, but it's now more clear that Allegany is second (do they have a trauma center?), followed by a few counties on the Eastern shore. 
...
# Can you establish intentional bins instead of using a log scale?

### Task: Run code
**Task** Run the following code. Describe the output in the space below. What patterns do you see?
**Answer** Now we see more differentiation among the counties that were previously lumped into the 200-beds-per-100k category. It looks like Western Maryland has a rather high beds-per-100,000 rate, possibly serving the nearby populations in West Virginia and Pennsylvania. Baltimore still ranks at the top. The regions near DC have a higher rate of beds for the population than Southern Maryland. I would plot trauma centers on top of this map to better inform the beds-per-people information. 
