...
**Answer** Metadata documentation for the North American Industry Classification System. 
...
**Answer** An array for the page <head> tag and a second array for the page <body> tag.
...
**Answer** a more nicely formatted description of the document's nodes (head and body)
...
**Answer** Looks like R returned a list of the lists (1 through six), as well as a separate dataframe for each table on the html page. I can click through each of the tbl_df icons to see each table converted to a dataframe (with appropriately labeled rows, columns, and set of values).
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** A list of the six table dataframes with row-by-column size information. 
...
**Answer** A table of the NAICS version history with implementation and release dates.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** The table of NAICS sector codes and descriptions
...
**Answer** a display of the NAICS sector codes and descriptions
...
**Answer**same dataframe, but this time with lowercased column headers and the source URL removed from the last row
...
**Answer** the tidied table produced as the final step in scraping, isolating and cleaning the NAICS sector list
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Many tables of data describing the utilites sector (employment, unemployment, wages, union membership, job trends, and more!). The statistics appear to cover the last four time units available, and the time unit varies from trailing four months to trailing for years in some cases. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** The head and body nodes returned as lists.
...
**Answer** A list of four lists that hold the contents of the Employment and Unemployment table.

[Note to self: This is the id for the entire table, which is extended for the elements in the table. Will html_element take any id or unique element? Could we have theoretically put "@header=" with the value in the xpath argument to retrieve only the relevant number?]
...
**Answer** I see a representation of the table with six rows in the first column ("Data Series") and four rows, including the "preliminary" footnote as a row, in the other columns, though the dataframe is described as 6x6. (Display is a little wacky and makes it hard to confirm values in the correct row.)
...
**Answer** A 1x2 dataframe that shows seasonally adjusted employment for June 2021.

[Note to self: slice(row) syntax + select for column roughly equivalent to [r:c] slicing]
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Same statistical read-out as Utilities but for Mining, Quarrying and Oil and Gas Extraction.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Same statistical page, this time for Construction.
...
**Task** Run the following code to print sectors. Briefly describe the output that appears below the codeblock. The list of sectors.
**Answer**
```{r}
...
**Answer** Nothing! But I have list_of_sectors in the Values category of my Environment. 
```{r}
...
**Answer** List of all sectors contained in "list_of_sectors," same as prior "print each separately" list above. 
...
**Answer** The code throws an error: "object 'sector_list' not found."
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** Same output as "for (sector in list_of_sectors) for loop above. 
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** Code block throws an error: "object 'sector_name' not found."
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** A list of all numbers 1 through 10 inclusive.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** 20x2 dataframe containing the NAICS sector codes and descriptions
...
**Answer** 20 (count of rows in naics_industry dataframe)
...
**Answer** A list of numbers 1 through 20.
...
**Answer** We get the single row seasonally adjusted employment for Utilities in June 2021.
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** 20x2 dataframe of NAICS sector codes and descriptions
```{r}
# Define url of page we want to scrape
...
**Answer** A 20x3 dataframe that updates the NAICS sector and description dataframe with a new column that contains a unique BLS URL for each sector code. 

[Note to self: Is this paste-letter-o or paste-zero?]
...
**Answer** A 20x4 dataframe that adds a column for the Unique table id for each sector, in addition to the sector code, description and BLS URL. 
...
**Answer** A 19x4 dataframe that removed the row from the prior table that refered to sector 92, public administration.  
...
**Answer** 19 separate dataframes, each containing one row from the NAICS table with URLs and table IDs and without public administration.  
...
We're going to do something similar with the xpath for our employment table by using the information in the sector_xpath_id column.
...
**Answer** A list that includes row number, URL and xpath for each NAICS code.

[Note to self: What is the resource for getting the syntax of xpaths?]
```{r}
...
**Answer** I got 19 dataframes. The first contains unemployment, not employment information. The remainder start with employment but have non-identical sets of information.

[Note to self: How do you keep track of identifying which df belongs to which sector using this method?]
[This is a nested list because it was all generated by the same for loop? Or some other reason?]
[Top of this code block repeats the "let's see what we're doing" code block above.]
...
**Answer** A set of 19 dataframes. The first (as foreshadowed above) contains unemployment rate. The remainder contain one row for seasonally adjusted employment.
...
**Answer** A set of 19 dataframes, each with the NAICS sector code, description, relevant URL and table ID added as columns. This means I can now identify Agriculture as the offending NAICS sector that has a nonstandard webpage and displays unemployment rate instead of seasonally adjusted employment. Also, we seem to have lost the column selector, so I have all available columns in all dataframes. 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** 19 standardized 1x3 dataframes that show NAICS sector code, description and June 2021 employment, with our friend the Agriculture Unemployment Rate as the interloper in the first dataframe. 

[Why aren't 19 dataframes represented in the employment_info data in the environment? Is "print" the thing that's producing all of these dataframes for view? It appears the for loop replaces each dataframe with the one that follows, leaving only the last in the end.]
...
**Answer** I get a 19x3 dataframe that shows the combined list of NAICS sector codes, descriptions, and June 2021 employment. All 7.5 agriculture workers rejoice!

[Note to self: Last step creates new dataframe and fills it]
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This is the mining employment table.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** First table on this page is unemployment, and there's no employment table to be had except a 2020 summary statistic. 
...
**Answer** The 7.5 theoretical agriculture works have been replaced by NA in the June 2021 employment column.

[Note to self: could you have covered the public administration values here by saying na_if(jun_2021_employees)<10000 or some other reasonable number?]
