...
**Answer** https://www.datawrapper.de/_/rfXAh/
...
**Answer** The code below groups loans by counties and then counts to give an output of the number of loans per county. 
...
**Answer** The code drops any county or jurisdiction where the number of loans is 6 or fewer.
...
**Answer** The code created a new column, fips_code, that concatenated the state and county FIPS codes to create the 5-digit FIPS code for each county. 
...
**Answer** The new file merges the Maryland PPP data with the Maryland FIPS identifiers to create an 8-column dataframe with original PPP county names and loan counts, plus all the geographic identifiers. 
...
**md_counties join didn't drop the extra columns, so fips_code = H**
...
**Answer** Now I see an slightly off-kilter but colorful Maryland with the rest of the surrounding US removed.
...
**Answer** The tooltips now show a human-readable county name instead of the FIPS code along with the count of loan in standard human-readable numeric format.  

**Note: {{ FORMAT(count, "0,0") }} seems to give the same output result as {{ FORMAT(count, "0,0.[00]")}} for integer data in this instance.
...
**Answer** https://www.datawrapper.de/_/164ty/
