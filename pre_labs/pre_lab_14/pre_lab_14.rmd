...
**Answer** We have 152,000-and-some tweets, each with with a Unique ID, a date stamp, twitter handle (or whatever the kids call it), an H or S for House or Senate, a state, district number, a title (differentiates candidate/campaign account from elected official), first name, last name, gender and political party.  
...
**Answer** The result is a one-column dataframe that's effectively list of more than 5 million words and numbers.

```{r}
unique_words <- covid_tweets %>% 
  select(content) %>%
...
**Answer**  We have a chart that shows the English language is full of articles and pronouns, also COVID-19.
...
**Answer** Now we see covid-related words (including pandemic, coronavirus, vaccine) dominating the list, with other plausably related words (health, relief).
...
**Answer** The potentially meaningful differences include "vaccine" and "vaccines" in the 2021 list, omitted from the 2020 list. "Crisis" has dropped off the list in 2021. ("Testing" not on my list.)

# tally(sort=TRUE)
# count observations by group
# https://www.rdocumentation.org/packages/dplyr/versions/0.5.0/topics/tally
# sort TRUE = desc
...
**Task** Run the following code and describe the results.
**Answer** Now we have a set of 2-word phrases where 'covid 19' shows up 7 times more freqently thanthe next more frequent phase. The phrases are dominated by the pandemic, including references to related legislation, plus "town hall." 

# Why not remove the stop words before creating the bigrams? Is this a different result?
...
**Task** Run the following code and describe the results.
**Answer** Now we have a similar set of words but with three phrases that include "vaccine" and we lost "town hall."
...
**Task** Run the following code and describe the results.
**Answer** Possibly meaningful differences -- Republican list references CARES Act by name and American people; the Democratic list references public health and social distancing. 
...
**Task** Run the following code and describe the results.
**Answer** The Democratic and Republican candidates each have the opposition in their top-10 lists (President Trump for Democrats and Joe Biden / Biden administration for Republicans). Unique items on the Democratic list include nursing homes, god bless, and social distancing, but nothing related to vaccines (interesting). The Republican list includes both covid vaccines and vaccine mandate, as well as illegal immigrants.  
...
**Answer** The results produce unique words with counts and a sentiment, where the sentiment seems to reflect the dictionary meaning of the word -- which it would almost have to without any context. (Interesting that this runs sentiment analysis on a word and not on a phrase or sentence.)
