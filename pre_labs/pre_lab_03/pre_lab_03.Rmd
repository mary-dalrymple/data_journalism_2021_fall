...
**Task** Run the following codeblock to glimpse the data.  What data type is the "amount" field?  What data type is the "name" field? What data type is the id column? 
**Answer** amount = numeric (dbl); name = character; id = character
...
#why is this syntax as.data_type?
...
**Answer**: 192,959 NA franchises; 275 Subway
...
**Answer**: It appears all loans where forgiveness_amount = NA, the forgiveness_date is also NA. We don't see any loans where forgiveness_amount is NA and there's an actual date, for example. This suggests consistency in the forgiveness data (and suggests to me that there are good screen-level controls on how this data is entered into the SBA database).
...
**Answer** $10 million does not seem suspcious because it's at the maximum allowed loan amount. $6 is odd but not necessarily suspicious because people will be people. But, I'd look at the distribution of very small dollar loans and consider whether to lop off all very small dollar loans from analysis. How many loans are under $100? Are there enough that they skew the averages? What's up with businesses applying for such tiny loans -- maybe that's a story? 
...
**Answer** Yes. For whatever reason, these businesses appear to have applied for $6 and $78 in payroll amounts. The value in the amount column does not appear to be a data entry error.
...
**Answer**'Amount' is a character, so we're not able to run math operations against that data (such as max, min, mean). 'City' shows multiple formats for Arnold that will prevent accurate grouping. The combination of 5- and 9-digit ZIP codes in the 'zip' column will cause a grouping problem similar to the formatting issues in 'city.' There are also duplicate rows that will introduce errors in any summary operations.
...
**Answer**It inserted an 'x' in front of the 1 to make the column header 'x1_id.'
...
**Answer**It's throwing an error on trying to do a math operator on character data. It's akin to to adding or subtracting data like 'name.' It's a nonsense operation.
...
**Answer** 1 pair of duplicates for Stuart Jones LLC
...
**Answer** 
#str_sub is syntax for string subset
...
**Task** Run the following codeblock to use str_to_title() to standarize capitalization in the "city" field. How many mispellings of Arnold remain after running this code? Answer below.
**Answer** Two; we still have Arnld and Anold.
...
**Answer** Only one, Arnld.

#Why do you add the TRUE ~ city to the case_when? 
#This just means any other value not specified should be replaced with itself (no change).
#Especially important when you're creating a new column, so that you carryforward all the unchanged values.
