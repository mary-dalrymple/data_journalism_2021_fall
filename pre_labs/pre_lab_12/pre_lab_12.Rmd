...
**Answer** We get a dataframe with 13 columns and 8,530 rows. We appear to have incremental unique IDs, a datetime stamp (always 1500) once per day, location by county, and a whole series of dosing columns for new and cumulative covid vaccines by dose. 
...
**Answer** We have Maryland's 23 jurisdictions, plus Unknown (336 values), plus NA (201 values).
...
**Answer** The first row shows 8000 first_dose_daily and first_dose_cumulative on 1/8/21. The counts increase slowly until 4/13/21, when first_dose_daily jumps to 84,295. I'm going to guess this is aggregated data for all Maryland jurisdictions, but I'd want to check that with some math before running with this idea. It's also possible these are institutional populations, like nursing homes and prisons? 
...
**Answer** The bottom ranked date is numbered 254, which means that most recent 7 to 10 days rank toward the bottom. 
...
# output tells us the difference in new vaccinations on the latest date compared with the same day a week ago, but this is incident data not cumulative data
...
**Answer** We can see the rise in vaccinations as they became publicly available, the peak over the late spring and early summer, and then the pattern establishing a more even rate with a slight increase in early September (back to school vaccines?). We can also see what appears to be the weekend dips in either vaccinations or vaccine data reporting.
...
# strip the '-' sign to remove the double negative; specify newly fully vaccinated
...
**Answer** We have a pile of time series lines depicting cumulative case counts by county. 
...
**Task** Run the following code and describe what the resulting chart shows
**Answer** Another stack of time series graphs with the initial runup in cases more exaggerated and one of the counties (I can't tell by color) spiking up and down at the bottom of the graph. 
...
**Answer** We have a colored map of Maryland (colored pretty similarly to a heatmap) that shows PG with the most (new? active?) COVID cases, followed by Montgomery and Baltimore counties. 
